...
exports.eliminarDia = async(req, res) => {
    idDia = req.params.id;
    await diasModel.deleteOne({_id: idDia})
    .then(()=>{
        res.status(201).send({message: "Se elimino el dia exitosamente."});
    })
    .catch((err)=>{
...
exports.actualizarDia = async(req, res) => {
    ...
    await diasModel.updateOne({_id: idDia}, dia, {
        new: true
    })
    .then(()=>{
        res.status(201).send({message: "Se actualizo el dia exitosamente."});
    })
    .catch((err)=>{
