...
exports.crearOfertaAcademica = async (req, res) => {
  ...
  if (JSON.stringify(modelKeys) != JSON.stringify(keys))
    return res
      .status(400)
      .send({
        message: "La oferta no cuenta con todos los campos requeridos.",
      });

  records.creadoPor = currentUserId;

  await ofertaAcademicaModel
    .insertMany(records)
    .then(() => {
      res
        .status(201)
        .send({ message: "Se registro la oferta academica exitosamente." });
    })
    .catch((err) => {
      console.log(err);
      if (err.code === 11000) {
        res
          ...
          .send({
            message:
              "No se permite clases ofertadas duplicadas, valide y corriga el archivo.",
          });
      } else {
        res
          .status(400)
          .send({ message: "No pudo registrar la oferta academica." });
      }
    });
};
