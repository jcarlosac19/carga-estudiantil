...
exports.crearTrimestre = async (req, res) => {
  ...
  if (!anio)
    return res.status(400).send({ message: "Debe de especificar un año." });
  if (!trimestre)
    return res
      .status(400)
      .send({ message: "Debe de especificar un trimestre." });
...
exports.obtenerTrimestres = async (req, res) => {
  await trismestresModel
    .find({})
    .then((trimestres) => {
      res.status(200).send(trimestres);
    })
    .catch((err) => {
      res
        .status(400)
        .send({ message: "No se pudieron obtener los trimestres." });
    });
};
  ...
  try {
    ...
    if (!id)
      return res
        .status(400)
        .send({
          message: "Debe de especificar el trimestre que desea eliminar.",
        });

    let ofertaAcademica = await ofertaAcademicaModel
      .find({ trimestre: id })
      .select("_id")
      .lean()
      .exec();
...
exports.actualizarTrimestre = async (req, res) => {
  ...
  if (!id)
    return res
      .status(400)
      .send({
        message: "Debe de especificar el trimestre que desea actualizar.",
      });
  let trimestreLista = await trismestresModel
    .find({ puedeMatricular: true })
    .lean()
    .exec();
  const hasActiveEnrollment = trimestreLista.length > 0 && puedeMatricular;
  if (hasActiveEnrollment === true)
    return res
      .status(500)
      .send({
        message:
          "Debe desactivar todos los trimestres con pre-matricula abierta antes de poder realizar esta accion.",
      });
  ...
  await trismestresModel
    ...
    .then(() => {
      res
        .status(200)
        .send({ message: "Se actualizo el trimestre exitosamente." });
    })
    .catch((err) => {
